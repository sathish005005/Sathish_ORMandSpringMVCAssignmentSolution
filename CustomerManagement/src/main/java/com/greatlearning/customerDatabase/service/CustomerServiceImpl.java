...
public class CustomerServiceImpl implements CustomerService {
	...
	public List<Customer> searchBy(String firstname, String lastname) {
		...
		if (firstname.length() != 0 && lastname.length() != 0)
			query = "from Customer where name like '%" + firstname + "%' or author like '%" + lastname + "%'";
		else if (firstname.length() != 0)
			query = "from Customer where name like '%" + firstname + "%'";
		else if (lastname.length() != 0)
			query = "from Customer where author like '%" + lastname + "%'";
		else
